...
class RelTime(RelativismData):
    ...
    def __add__(self, other):
        if (not isinstance(other, RelTime)):
            other = Conversion.valid_beatsec(other)
...
class RelSamps(RelTime):
    ...
    def __init__(self, samples_val):
        super().__init__(samples_val, "sample")
...
class Conversion:
    ...
    def valid_beatsec(beat):
        ...
        Conversion._get_beat_frac(beat_type)
    ...
    def beats_to_samps(num, beat_type):
        ...
        frac = Conversion._get_beat_frac(beat_type)
        return num * frac * sec_per_beat * Relativism.rate()

    @staticmethod
    def _get_beat_frac(beat):
        fracs = Conversion._get_beat_frac_tables()
        for i in fracs:
            if beat == i[0]:
                return i[1]
        raise TypeError
    ...
    def freq(note):
        ...
        return Conversion.valid_freq(note)
    ...
    def valid_freq(note):
        ...
        if re.match(r"^[a-g][b#]{0,1}([0-9]|10)$", note) is None:
            ...
            raise TypeError
        # note is regular
        return Conversion._get_freq(note)
    ...
    def _get_freq(note):
        ...
        """
        freq_arr = Conversion._get_freq_table()
    ...
    def show_freq_table():
        ...
        for i in Conversion._get_freq_table():

...
class NameAndPath:
    ...
    def namepath_init(open_type):
        ...
        if proj_name is None:
            open_mode = NameAndPath.select_init_mode(open_type)
        else:
            open_mode = "o"
        ...
        while True:
            if open_mode == "c":
                try:
                    proj_name, proj_path = NameAndPath.select_create_name_path(open_type)
                    NameAndPath.verify_create_name(open_type, proj_name)
                ...
                proj_name = NameAndPath.select_open_name(open_type)
                try:
                    proj_path = NameAndPath.get_open_path(open_mode, open_type, proj_name, proj_path)
                    break
    ...
    def select_create_name_path(open_type):
        ...
        proj_name = input().lower().strip()
        proj_name = re.sub(r"\s+", "_", proj_name)
        proj_name = re.sub(r"[^_a-z0-9]", "", proj_name)
        proj_name = re.sub(r"_{2,}", "_", proj_name)
        NameAndPath.verify_create_name(open_type, proj_name)
